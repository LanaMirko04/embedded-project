...
static esp_err_t app_lvgl_main(void) {
  ...
  draw_first_screen(scr);
  /*
  lv_obj_set_style_bg_color(scr, lv_color_make(0xF1, 0xEC, 0xE6),
                            LV_STATE_DEFAULT);

  static lv_style_t test_style;
  lv_style_init(&test_style);
  lv_style_set_text_color(&test_style, lv_color_make(0x3E, 0x15, 0x00));
  lv_style_set_text_font(&test_style, &lv_font_montserrat_48);

  lv_obj_t *label = lv_label_create(scr);
  lv_label_set_text(label, "Sdrumo");
  lv_obj_add_style(label, &test_style, LV_STATE_DEFAULT);
  lv_obj_align(label, LV_ALIGN_CENTER, 0, 0);
*/
  /*
    lv_obj_t *labelR = lv_label_create(scr);
    lv_label_set_text(labelR, "Red");
    lv_obj_set_style_text_color(labelR, lv_color_make(0xff, 0, 0),
                                LV_STATE_DEFAULT);
    lv_obj_align(labelR, LV_ALIGN_TOP_MID, 0, 0);

    lv_obj_t *labelG = lv_label_create(scr);
    lv_label_set_text(labelG, "Green");
    lv_obj_set_style_text_color(labelG, lv_color_make(0, 0xff, 0),
                                LV_STATE_DEFAULT);
    lv_obj_align(labelG, LV_ALIGN_TOP_MID, 0, 32);

    lv_obj_t *labelB = lv_label_create(scr);
    lv_label_set_text(labelB, "Blue");
    lv_obj_set_style_text_color(labelB, lv_color_make(0, 0, 0xff),
                                LV_STATE_DEFAULT);
  ...
  */
  ...
  // lv_obj_add_event_cb(btn_counter, ui_event_Screen, LV_EVENT_ALL,
  // btn_counter);

  // lbl_counter = lv_label_create(btn_counter);
  /*
    lv_label_set_text(lbl_counter, "testing");
    lv_obj_set_style_text_color(lbl_counter, lv_color_make(248, 11, 181),
                                LV_STATE_DEFAULT);
