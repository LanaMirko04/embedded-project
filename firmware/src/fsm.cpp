 ...
 *                   - Result::INVALID_NEXT_STATE if the given state is not
 *                     related to the current one.
 */
typedef Result (*fsm_trans_fun)(enum FsmState next_state);
 ...
 */
static Result prv_fsm_init_trans_fun(enum FsmState next_state) {
    switch (next_state) {
        case FSM_STATE_INIT:
        ...
        default:
            ...
            result_set_err_msg("The state %s is not reachable from %s state.",
 ...
 */
static Result prv_fsm_wifi_connection_trans_fun(enum FsmState next_state) {
    switch (next_state) {
        case FSM_STATE_CHECK_CONFIG:
        case FSM_STATE_WIFI_CONNECTION:
        case FSM_STATE_ERROR:
 ...
 */
static Result prv_fsm_check_config_trans_fun(enum FsmState next_state) {
    switch (next_state) {
        case FSM_STATE_ERROR:
 ...
 */
static Result prv_fsm_load_config_trans_fun(enum FsmState next_state) {
    switch (next_state) {
        case FSM_STATE_ERROR:
 ...
 */
static Result prv_fsm_display_update_trans_fun(enum FsmState next_state) {
    switch (next_state) {
        case FSM_STATE_ERROR:
        case FSM_STATE_DISPLAY_UPDATE:
        case FSM_STATE_INIT:
 ...
 */
static Result prv_fsm_error_trans_fun(enum FsmState next_state) {
    switch (next_state) {
        case FSM_STATE_ERROR:
        case FSM_STATE_INIT:
            fsm_current_state = next_state;
