...
/* Persistent weather data — must be static so LVGL image pointers remain
 * valid after ui_load_screen_weather returns */
...
void ui_load_screen_weather(lv_obj_t *screen) {
    ...
    bool fresh = api_task_copy_weather(&s_weather);
    if (!fresh || api_task_is_server_down()) {
    ...
    lv_image_set_src(img_weath, &s_weather.today_weath_icon);
    ...
    lv_label_set_text(lbl_prob_rain, s_weather.today_rain);
    ...
    lv_label_set_text(lbl_humidity, s_weather.today_hummidity);
    ...
    lv_label_set_text(lbl_wind, s_weather.today_wind);
    ...
    lv_label_set_text(lbl_day1, s_weather.day1);
    lv_obj_add_style(lbl_day1, &style_par, 0);
    
    lv_obj_t *img_weather1 = lv_image_create(rect_t1);
    lv_image_set_src(img_weather1, &s_weather.weather_1_icon);
    
    lv_obj_t *lbl_temp1 = lv_label_create(rect_t1);
    lv_label_set_text(lbl_temp1, s_weather.temp_1);
    ...
    lv_label_set_text(lbl_day2, s_weather.day2);
    lv_obj_add_style(lbl_day2, &style_par, 0);
    
    lv_obj_t *img_weather2 = lv_image_create(rect_t2);
    lv_image_set_src(img_weather2, &s_weather.weather_2_icon);
    
    lv_obj_t *lbl_temp2 = lv_label_create(rect_t2);
    lv_label_set_text(lbl_temp2, s_weather.temp_2);
    ...
    lv_label_set_text(lbl_day3, s_weather.day3);
    lv_obj_add_style(lbl_day3, &style_par, 0);
    
    lv_obj_t *img_weather3 = lv_image_create(rect_t3);
    lv_image_set_src(img_weather3, &s_weather.weather_3_icon);
    
    lv_obj_t *lbl_temp3 = lv_label_create(rect_t3);
    lv_label_set_text(lbl_temp3, s_weather.temp_3);
