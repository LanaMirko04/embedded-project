...
static uint16_t map(uint16_t n, uint16_t in_min, uint16_t in_max,
                    uint16_t out_min, uint16_t out_max) {
  uint16_t value = (n - in_min) * (out_max - out_min) / (in_max - in_min);
  return (value < out_min) ? out_min : ((value > out_max) ? out_max : value);
}

static void process_coordinates(esp_lcd_touch_handle_t tp, uint16_t *x,
                                uint16_t *y, uint16_t *strength,
                                uint8_t *point_num, uint8_t max_point_num) {
  //ESP_LOGI("demo", "touched pre  %d, %d", *x, *y);

  uint16_t x_temp = *x;
  uint16_t y_temp = *y;

  *x = map(y_temp, TOUCH_Y_RES_MIN, TOUCH_Y_RES_MAX, 0, LCD_H_RES);
  *y = map(x_temp, TOUCH_X_RES_MIN, TOUCH_X_RES_MAX, 0, LCD_V_RES);
  ESP_LOGI("demo", "touched post %d, %d", *x, *y);
  //  ESP_LOGI("demo", " ");
}
...
esp_err_t touch_init(esp_lcd_touch_handle_t *tp) {
  ...
  ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)TOUCH_SPI,
